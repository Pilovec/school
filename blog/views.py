        ...
        if on_request.status_code == 200:
            ...
            for news_item in news:
                ...
                if news_item.find('a') is not None:
                    date_post = news_item.find(
                        'span', attrs={'class': 'bdate'}).text
                    if date_post.lstrip(' ') == current_date:
                        img = news_item.find(
                            'img', attrs={'class': 'h92'})['src']
                        title = news_item.find('a').text
                        title_link = news_item.find('a')['href']
                        short_descr = news_item.find(
                            'span', attrs={'class': 'btxt'}).text
                        ...
                        add.source = Source.objects.filter(
                            name='osvita.ua').first()
                        add.save()
                    else:
                        continue
                else:
